    ...
    foreach ($rule in $asrRuleNames.GetEnumerator()) {
        ...
        if ($ruleIndex -ge 0) {
            switch ($asrActions[$ruleIndex]) {
                0 { Write-Host ("{0}: Not Configured" -f $rule.Value) -ForegroundColor Red }
                1 { Write-Host ("{0}: Enabled" -f $rule.Value) -ForegroundColor Green }
                2 { Write-Host ("{0}: Audit" -f $rule.Value) -ForegroundColor White }
